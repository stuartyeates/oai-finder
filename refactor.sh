...
search_for_software () {
    ...
    for url in `cat ojs-terms.*.utf8 islandora-terms.*.utf8 etd-db-terms.*.utf8 vital-terms.*.utf8 dspace-terms.*.utf8 eprints-terms.*.utf8 greenstone-terms.*.utf8| sort | uniq| shuf`; do 
	(bing_search "${url}" &)
	(google_search "${url}" &)
	(sogou_search  "${url}" &)
